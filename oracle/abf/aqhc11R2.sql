...
select 'AQ Health Check (V11.2.0R3) for '||global_name||' on Instance='||instance_name||' generated: '||sysdate o  from global_name, v$instance;
...
from DBA_SCHEDULER_JOBS s LEFT OUTER JOIN DBA_SCHEDULER_RUNNING_JOBS r
    ON (s.job_name=r.job_name and s.owner=r.owner)
where s.JOB_NAME LIKE '%register_driver%'
order by s.instance_id, s.job_name;
