...
AND atc.column_name like '%LPN_ID%'
		   AND atc.table_name not like '%BACKUP%'
		   AND atc.table_name not like '%BK%'
		   AND atc.table_name not like '%_BKP'
		  AND substr(atc.table_name,1,5) <> 'MMTT_'
		AND substr(atc.table_name,1,5) <> 'MOQD_'
		AND substr(atc.table_name,1,4) <> 'MSN_'
		AND substr(atc.table_name,1,9) <> 'MTL_MMTT_'
		AND substr(atc.table_name,1,4) <> 'MMT_'
		AND substr(atc.table_name,1,4) <> 'MTI_'
		AND substr(atc.table_name,1,4) <> 'WLC_'
		AND substr(atc.table_name,1,5) <> 'WLPN_'
		AND substr(atc.table_name,1,3) <> 'XX_'
		AND substr(atc.table_name,1,3) <> 'RT_'
		AND substr(atc.table_name,1,4) <> 'RSL_'
		AND substr(atc.table_name,1,23) <> 'MTL_MATERIAL_TRANS_TEMP'
		AND substr(atc.table_name,1,19) <> 'MTL_MATERIAL_TRANS_'
		AND substr(atc.table_name,1,1) NOT IN ('B','G','A','C')
...
BEGIN  -- MAIN
    ...
    --BEGIN 
      ...
      LOOP
          if tabelle.table_name = 'WMS_LPN_CONTENTS_BU' or tabelle.table_name = 'WMS_CARTONIZATION_TEMP'
          then
