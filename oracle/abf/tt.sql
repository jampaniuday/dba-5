...
ORDER BY 4 DESC;
prompt TOTAL ALOCADO
select trunc(round(sum(bytes)/1024/1024/1024,2)) GBytes FROM dba_data_files;
prompt TOTAL UTILIZADO
select a.Gb - b.Gb GBytes
from (select trunc(round(sum(bytes)/1024/1024/1024,2)) Gb FROM dba_data_files) a,
(select trunc(round(sum(bytes)/1024/1024/1024,2)) Gb FROM dba_free_space) b;
select instance_name,host_name,to_char(sysdate,'dd/mm/yy hh24:mi') from v$instance;
