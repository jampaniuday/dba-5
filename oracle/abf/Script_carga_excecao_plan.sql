...
declare
   cursor c_itens is
          select a.organization_code , a.item_number , a.plan_mrp , a.wip ,a.status , a.msg_erro , ood.organization_id
          from bolinf.xxinv_carga_excecao a, apps.org_organization_definitions ood
          where a.status='PENDENTE_PL'
...
begin
   ...
   loop
      ...
      IF l_msg_erro IS NOT NULL THEN 
         ...
         where item_number = r_itens.item_number
         and organization_code = r_itens.organization_code and status = 'PENDENTE_PL';
      END IF;
      l_msg_erro := NULL;
      COMMIT;
   end loop;
