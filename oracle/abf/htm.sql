...
# CHANGE HISTORY: 
 ...
 15-MAR-12 Set WMS Information to Default to 'N' as the file         rijames
...
UTL_FILE.PUT_LINE(handle,'&std &sh LINE_ID &dh LINE NUMBER &dh ITEM &dh ORDER QTY &dh UOM &dh LINE STATUS &dh SHIP QTY &dh ');
...
begin
 ...
 loop
   utl_file.put_line(handle,'&sld'||n(ll.LINE_ID)||'&d'||n(ll.LINE_NUM)||'&d'||n(ll.ITEM)||'&d');
...
-- Append to output file
  select substr(value,1,instr(value||',',',')-1) into dirname
    from v$parameter where name = 'utl_file_dir';
  handle := UTL_FILE.FOPEN(dirname,dirname||'/&out_file','A',32000);
...
UTL_FILE.PUT_LINE(handle,'&std &sh WARNING &dh DELIVERY DETAIL_ID &dh RELEASE STATUS &dhr INI &dh OMI &dh MOVE_ORDER LINE_ID &dh ');
...
UTL_FILE.PUT_LINE(handle,'SHIP SET &dh SHIP MOD_CMP &dh TOP_MODEL LINE_ID &dh DISCRETE SRC_REQ_QTY &dh ');
...
UTL_FILE.PUT_LINE(handle,'QUALITY CTRL_QTY2 &dh DISCRETE SCHD_QTY &dh SUBLOT NUMBER &dh LAST UPDATE_DATE &eh');
...
if UPPER(nvl('&do_analysis','Y')) = 'Y' then
   ...
   if c_lines >= 35 then
     ...
     UTL_FILE.PUT_LINE(handle,'DELIVERY ID &dh TRIP_ID &dh LINE &dh LINE_ID &dh ITEM_ID &dh ITEM &dh SOURCE REQ_QTY &dh ');
     UTL_FILE.PUT_LINE(handle,'SOURCE REQ_UOM &dh REQUEST QTY &dh REQUEST UOM &dh PICKED QTY &dh SHIPPED QTY &dh ');
     UTL_FILE.PUT_LINE(handle,'DELIVER QTY &dh CANCEL QTY &dh PICK FLAG &dh INI &dh OMI &dh STA &dh STB &dh SHIP FROM_ID &dh ');
     UTL_FILE.PUT_LINE(handle,'SHIP TO_ID &dh WH_ID &dh CURRENT SUBINV &dh ORG SUBINV &dh REV &dh LOT &dh SERIAL &dh ');
     UTL_FILE.PUT_LINE(handle,'TRANSAC TEMP_ID &dh LOCATOR ID &dh SHIP METHOD &dh CREATE DATE &dh SPLIT_FROM DEL_DET_ID &dh ');
     UTL_FILE.PUT_LINE(handle,'SHIP SET &dh SHIP MOD_CMP &dh TOP_MODEL LINE_ID &dh DISCRETE SRC_REQ_QTY &dh ');
     UTL_FILE.PUT_LINE(handle,'DISCRETE SRC_REQ_UOM &dh DISCRETE REQ_QTY &dh DISCRETE REQ_UOM &dh DISCRETE PICK_QTY &dh ');
   ...
   utl_file.put_line(handle,n(dd.TOP_MO_LIN)||'&d');
   ...
   utl_file.put_line(handle,n(dd.slotno)||'&d'||n(dd.upd_date)||'&el');
...
begin
 ...
 loop
   ...
   utl_file.put_line(handle,n(wddc.CONT_TYPE)||'&d'||n(wddc.NET_WT)||'&d');
   utl_file.put_line(handle,n(wddc.GRS_WT)||'&d'||n(wddc.VOL)||'&d');
   utl_file.put_line(handle,n(wddc.WT_UOM)||'&d'||n(wddc.SRQ_Q)||'&d');
